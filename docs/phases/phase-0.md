...
**Update (post-Phase-4, 2026-09-10):** the corpus is now captured on the **real
Phase-4 answer sheets** — `corpus/_source/sheet_{a,b,c,d}_*.pdf`, generated by
`python scripts/make_corpus_sheets.py` (geometry from
`config/sheet_template.json`). The pre-Phase-4 `throwaway_v0` is retired for the
capture pass — a corpus shot on it could not validate the real bubble-crop
geometry. Full protocol: `corpus/README.md`.

**Executed by the user** (physical steps — Claude cannot do these):
1. Print the four `corpus/_source/sheet_*.pdf` masters on a laser printer (A4,
   100% scale), several copies each.
2. Photocopy a handful of copies 1 and 2 generations deep (before filling).
...
- [x] 0.2 throwaway sheet — generated; QR decodes from rasterized page  (commit 777bc63)
      **superseded 2026-09-10** by `scripts/make_corpus_sheets.py` → real Phase-4
      masters `corpus/_source/sheet_{a,b,c,d}_*` (Phase 4 finished the renderer)
